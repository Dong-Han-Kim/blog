...
React가 이전 Virtual DOM과 새 Virtual DOM을 비교할 때, 모든 노드를 완벽하게 비교하려면 시간 복잡도가 **O(n³)**이다. 노드가 1,000개면 비교 연산이 10억 번이 되는 셈이다.

React는 이를 해결하기 위해 **두 가지 휴리스틱(가정)**을 적용하여 복잡도를 **O(n)**으로 줄였다.
